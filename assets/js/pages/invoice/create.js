...
$(() => {
                        ...
                        if(type == 'comp') {
                          ...
                          console.log('composant');
                            ...
                            $('#hiddenComponent .invoicesComponents-product-'+id).each(function() {
                                ...
                                let nameFr   = $('#nameFr-invoiceComponent-'+type+'-'+id+'-'+k).text();
                                let nameEn   = $('#nameEn-invoiceComponent-'+type+'-'+id+'-'+k).val();
                                let priceHt  = $('#priceHt-invoiceComponent-'+type+'-'+id+'-'+k).text();
                                let quantity = $('#quantity-invoiceComponent-'+type+'-'+id+'-'+k).val();
                                let vat      = $('#vat-invoiceComponent-'+type+'-'+id+'-'+k).text();

                                let priceVat = $('#priceVat-invoiceComponent-'+type+'-'+id+'-'+k).text();
                                let priceTtc = $('#priceTtc-invoiceComponent-'+type+'-'+id+'-'+k).val();
                                let totalHt  = $('#totalHt-invoiceComponent-'+type+'-'+id+'-'+k).text();
                                let totalTtc = $('#totalTtc-invoiceComponent-'+type+'-'+id+'-'+k).text();
      ...
      let url = `product/display/${product_id}`;
        $.ajax({
            ...
            success(product) {
                ...
                $('#hiddenComponent').append("<div class='invoicesComponents-product-"+product_id+"'><hr/><b  style='color: darkblue'>"+product.nameFr+" - Détails des composants </b></div>");
                ...
                for(j = 0; j < product.components.length; j++)
                {
                    ...
                    "<div class='hiddenComponent' style='display: block; font-weight: bold; padding-left: 40px' name='nameFr-invoiceComponent-product-"+product_id+"' id='nameFr-invoiceComponent-product-"+product_id+"-"+j+"'>"+component['nameFr']+"</div>"+
                    '<div style="display: flex; justify-content: space-between; flex-wrap: wrap; padding-left: 40px">'+
                        "<input type='hidden' class='hiddenComponent' name='nameEn-invoiceComponent-product-"+product_id+"'  id='nameEn-invoiceComponent-product-"+product_id+"-"+j+"' value='"+component['nameEn']+"' />"+
                        
                        "<div>PU TTC en €uros <input data-key="+j+" data-productid = "+product_id+" type='text' class='hiddenComponent changeComponentValue' style='display: block; width: 100px' name='priceTtc-invoiceComponent-product-"+product_id+"'  id='priceTtc-invoiceComponent-product-"+product_id+"-"+j+"' value='"+component['priceTtc']+"' /></div>"+

                        "<div>Quantité <input type='text' data-key="+j+" data-productid = "+product_id+" class='hiddenComponent changeComponentValue' style='display: block; width: 50px' name='quantity-invoiceComponent-product-"+product_id+"'  id='quantity-invoiceComponent-product-"+product_id+"-"+j+"' value='"+component['quantity']+"' /></div>"+
                        "<div>% TVA<div class='hiddenComponent' style='display: block' name='vat-invoiceComponent-product-"+product_id+"'  id='vat-invoiceComponent-product-"+product_id+"-"+j+"'>"+component['vat']+"</div></div>"+
                        "<div>Unité TVA<div class='hiddenComponent' style='display: block' name='priceVat-invoiceComponent-product-"+product_id+"'  id='priceVat-invoiceComponent-product-"+product_id+"-"+j+"'>"+component['priceVat']+"</div></div>"+
                               
                        "<div>PU HT en €uros<div class='hiddenComponent' style='display: block'  name='priceHt-invoiceComponent-product-"+product_id+"'  id='priceHt-invoiceComponent-product-"+product_id+"-"+j+"' >"+component['priceHt']+"</div></div>"+

                        "<div>Total HT<div class='hiddenComponent' style='display: block' name='totalHt-invoiceComponent-product-"+product_id+"'  id='totalHt-invoiceComponent-product-"+product_id+"-"+j+"' >"+component['totalHt']+"</div></div>"+
                        "<div>Total TTC<div class='hiddenComponent' style='display: block'name='totalTtc-invoiceComponent-product-"+product_id+"'  id='totalTtc-invoiceComponent-product-"+product_id+"-"+j+"' >"+component['totalTtc']+"</div></div>"+
                    "</div></div>";
    ...
    function resetListenner()
    {
        ...
        $(".changeComponentValue").change(function() {
            ...
            // info component
            let priceTtc  = parseFloat(document.getElementById('priceTtc-invoiceComponent-product-'+product_id+'-'+key).value);
            let quantity = parseFloat(document.getElementById('quantity-invoiceComponent-product-'+product_id+'-'+key).value);
            let vat      = parseFloat(document.getElementById('vat-invoiceComponent-product-'+product_id+'-'+key).textContent);
        
            // info component
            let priceVat = document.getElementById('priceVat-invoiceComponent-product-'+product_id+'-'+key);
            let priceHt = document.getElementById('priceHt-invoiceComponent-product-'+product_id+'-'+key);
            let totalHt  = document.getElementById('totalHt-invoiceComponent-product-'+product_id+'-'+key);
            let totalTtc = document.getElementById('totalTtc-invoiceComponent-product-'+product_id+'-'+key);
            let firstTotalTtc = parseFloat(totalTtc.textContent);
        
            // info priduct
            let productUTtc = document.getElementById('product-uTtc-'+product_id);
        
            let priceProductUTtc = parseFloat(productUTtc.textContent);
        
            // update componennt
            priceHt.textContent = (priceTtc*100/(100+vat)).toFixed(2); // ok
            totalHt.textContent  = (priceTtc*100/(100+vat)*quantity).toFixed(2); // ok
            totalTtc.textContent = priceTtc*quantity; // ok
            priceVat.textContent = ( priceTtc-(priceTtc*100/(100+vat)) ).toFixed(2);

            console.log(priceVat);
            console.log((priceTtc*100/(100+vat)).toFixed(2));


            let balance = parseFloat((totalTtc.textContent) - firstTotalTtc).toFixed(2) ;
        
            productUTtc.textContent = (parseFloat(priceProductUTtc) + parseFloat(balance)).toFixed(2);
        ...
        $('.delete-product-line').click(function() {
            ...
            $('.invoicesComponents-product-'+id).remove();
        ...
        $('.comp-values').change(function() {
            ...
            let uHt    = (uTtc*100/(100+vat)).toFixed(2);
    ...
    function calculeTotalProductLine(product_id){
        ...
        console.log('calculeTotalProductLine');
