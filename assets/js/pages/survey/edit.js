            ...
            success(json) {
                ...
                if (json.status == true) {
                    $(`.chapter`).each(function () {
                        if($(this).data('edit')) {
                            editChapter(this, $(this).attr('data-edit'));
                        } else {
...
const editChapter = (e, chapterId) => {
    ...
    let data = { name, description, type: typeChapter, is_active };

    $.ajax({
        type: "POST",
        url: urlRequest,
        data: { type, url: 'survey/chapter/modify/' + chapterId, data },
        dataType: "json",
        beforeSend() {
            $("#surveyForm [type=submit]")
                .attr("disabled", true)
        ...
        success(json) {
            ...
            if (json.status == true) {
                $(e).find('.question').each(function () {
                    if($(this).data('edit')) {
                        editQuestion(this, $(this).attr('data-edit'));
                    } else {
...
const editQuestion = (e, questionId) => {
    let type = "PUT";
    let name = $(e).find(".chapterQuestion").val();
    let description = $(e).find(".chapterQuestionDescription").val();

    let data = { name, description, scale: 5 };

    $.ajax({
        type: "POST",
        url: urlRequest,
        data: { type, url: 'survey/question/modify/' + questionId, data },
        dataType: "json",
        beforeSend() {
            $("#surveyForm [type=submit]")
                .attr("disabled", true)
