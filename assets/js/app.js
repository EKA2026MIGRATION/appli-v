...
$(document).ready(function() {
  localStorage.setItem("searchPage", "impossibletofindthistext");
  localStorage.setItem("searchPageNumber", 0);
  localStorage.setItem("searchPageResult", null);
});
...
/*
var toogleMfb = (data) => {
  if ($(data).attr("data-mfb-state") == "closed") {
    $(data).attr("data-mfb-state", "open");
  } else {
    $(data).attr("data-mfb-state", "closed");
  }
};
*/

function searchOnPageMobile() {
  let text = $(".searchFieldMobile").val();

  if (localStorage.getItem("searchPage") == text) {
    var i = 0;
    $(`span`).each(function() {
      const style = $(this).attr("style");

      if (style == "background-color: yellow;") {
        if (i == localStorage.getItem("searchPageNumber")) {
          ...
          var nb = i + 1;
          $(".nbResultActual").html(nb);
          localStorage.setItem("searchPageNumber", nb);
          return false;
        }
        i++;
      }
    ...
    var nbTotal = i + 1;
    if (parseInt(i) == parseInt(localStorage.getItem("searchPageResult"))) {
      localStorage.setItem("searchPage", "impossibletofindthistext");
      localStorage.setItem("searchPageNumber", 0);
      localStorage.setItem("searchPageResult", null);
      searchOnPageMobile();
    ...
    let text = $(".searchFieldMobile").val();
    localStorage.setItem("searchPage", text);
      ...
      $(`span`).each(function() {
        ...
        if (style == "background-color: yellow;") {
          if (i == 0) {
            ...
            localStorage.setItem("searchPageNumber", 1);
          }
          i++;
        }
      });

      localStorage.setItem("searchPageResult", i);

      if (i != 0) {
        $(".divResultSearchPage").show();
        $(".nbResultSearchPage").html(i);
        $(".nbResultActual").html("1");
      }

      if (i == 0) {
        toastr.error("Aucun résultat");
        $(".divResultSearchPage").hide();
        $(".nbResultSearchPage").html("");
        $(".nbResultActual").html("");
      }
    } else {
      toastr.error("Aucun résultat");
...
/*
  $(document).on("touchstart", function (e) {
      ...
      position = document.querySelector('.menu__left').scrollTop;

  }).bind("touchend", function (e) {
      var distance = touchStartPos - $(window).scrollTop();
      var distanceMenu = position - document.querySelector('.menu__left').scrollTop;

      if (distance > 20 || distance < -20) {
          e.preventDefault;
      }
      else if(distanceMenu > 20 || distanceMenu < -20)
      {
        e.preventDefault;
      }
      else
...
const mapsApiKey = $("#mapsApiKey").val();
const urlRequest = $("#urlRequest").val();
const urlSMS = $("#urlSMS").val();
const urlSendSMSFactor = $("#urlSendSMSFactor").val();
const urlPhoto = $("#urlPhoto").val();
const showLoader =
  '<div class="containerLoader"><div class="lds-roller"><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div></div>';
const photoProfilDefault = $("#photoProfilDefault").val();
const noPhoto = $("#noPhoto").val();
const tokenAuth = $("#tokenAuth").val();

const openRevealJS = (element) => {
  $(`#${element}`).foundation("open");
};

$(document).on("closeme.zf.reveal", "[data-reveal]", function() {
  var y = window.scrollY;
  const currentScroll = localStorage.getItem("currentScroll");

  if (currentScroll != null) {
    if (y == 0 && currentScroll != 0) {
      localStorage.setItem("currentScroll", currentScroll);
    } else {
      localStorage.setItem("currentScroll", y);
    }
  } else {
    localStorage.setItem("currentScroll", y);
  }
...
/*
  $('mfb-component__list a, .mfb-component__button--main').click(function(e){ 
    $('.top-bar').trigger('click');
  });
  */
/*
  $("[data-mfb-toggle]").click(function(e){ 
    if($(this).attr('data-mfb-state') == 'open') {
        $(this).attr('data-mfb-state', 'closed');
    } else {
        $(this).attr('data-mfb-state', 'open');
    }
  });
  */
...
$('th').click(function(e){ 
    ...
    var rows = table.find('tr.search:gt(0)').toArray().sort(comparer($(this).index()))
    this.asc = !this.asc
...
if (window.location.href.indexOf("iframe") > -1) {
  $("a").each(function() {
    ...
    $this.attr("href", _href + "iframe/yes/");
  });
}

const locationRedirect = (myUrl) => {
  if (myUrl == null) {
    location.reload();
  } else {
  ...
  $.fn.serializeToJSON = function(options) {
    const f = {
      ...
      getValue($input) {
        ...
        const floatCondition = this.settings.parseFloat.condition;
        if (
      ...
      createProperty(o, value, names, $input) {
        let navObj = o;
        ...
        for (let i = 0; i < names.length; i++) {
          ...
          if (i === names.length - 1) {
            ...
            const arrayKey = /\[\w+\]/g.exec(currentName);
            const isArray = arrayKey != null && arrayKey.length > 0;
            ...
            if (isArray) {
              ...
              const keyName = arrayKey[0].replace(/[\[\]]/g, "");
              currentName = keyName;
            }
      ...
      includeUncheckValues(selector, formAsArray) {
        $(":radio", selector).each(function() {
          const isUncheckRadio =
      ...
      serializer(selector) {
        ...
        const formAsArray = $(selector).serializeArray();
        this.includeUncheckValues(selector, formAsArray);

        const serializedObject = {};

        $.each(formAsArray, (i, item) => {
          const $input = $(`:input[name='${item.name}']`, selector);

          const value = self.getValue($input);
...
((win, doc) => {
  ...
  class ImageCompressor {
    ...
    firstCompress() {
      ...
      const toW = this.settings.toWidth;
      const toH = this.settings.toHeight;
      if (w < toW || h < toH) {
        return;
      }
      canvas.width = w;
    ...
    fitWidth() {
      ...
      const offsetX = 0;
      const offsetY = (this.settings.toHeight - compressedHeight) / 2;
      if (this.compressedCanv) {
        this.context.drawImage(
          this.compressedCanv,
          0,
    ...
    fitHeight() {
      ...
      const offsetY = 0;
      const offsetX = (this.settings.toWidth - compressedWidth) / 2;
      if (this.compressedCanv) {
        this.context.drawImage(
          this.compressedCanv,
          0,
    ...
    vReverse() {
      ...
      const pixels = imgData.data;
      const reversedData = this.context.createImageData(
        this.canvas.width,
        this.canvas.height
      );
      // var chunk = [];
