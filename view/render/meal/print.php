                                ...
                                <li>
                                    ...
                                    <?php if(key_exists($child->id, $mealChild)):?>
                                            ...
                                            <span style='font-style:italic'>
                                                <?= $params->age['child'][$child->firstname.' '.$child->lastname];?>
                                            </span>
                                            <?php if(isset($medical[$child->firstname.' '.$child->lastname])) echo "<br/><i style='font-size: 9px; color: red'>".$medical[$child->firstname.' '.$child->lastname].'</i>';?>
                                    <?php endif;?>
    ...
    <div>
        <ul id="withoutGroup">
          <?php foreach($presences as $presence):?>
                ...
                <?php if( isset($presence->child->childId) && isset($groupChild) && !key_exists($presence->child->childId, $groupChild) && !isset($arr[$presence->child->firstname.' '.$presence->child->lastname])):?>
                    
                    <!-- check time --->
                    <?php if(str_replace(':', '', $presence->start) < '120000' && str_replace(':', '', $presence->end) > "120000"):?>
                        <li>
                            <?= $presence->child->firstname.' '.$presence->child->lastname;?>
                            <?php if( strlen($presence->child->medical) > 0):?>
                                <i class="material-icons" style="color: darkblue" title="<?php echo $presence->child->medical;?>">local_hospital</i>
                            <?php endif;?>
                            <!-- chek if has  meal --->
                            <div class="lunchLine">
